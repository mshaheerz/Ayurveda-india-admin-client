...
function AddWellnessModal({ isOpen, onOpen, onOpenChange, token, refresh, setRefresh, onClose, mode, initialData, setInitialData }: AddCourseProps) {
    ...
    const removeTheoryModule = async (index: number, id: any) => {
        ...
        if (mode === "edit") {
            ...
            try {
                ...
                await axios.delete('/wellness/delete_module/', {
                    data: formData,
                    headers: {
    ...
    const onSubmit = async (data: any) => {
        ...
        let forms = {
            ...data,
            "wellness_modules": JSON.stringify(intprs),
        }
        // return ;
        ...
        if (mode === "edit") {
            ...
            try {
                const { data } = await axios.put(`/wellness/${initialData.id}/`, formData,
                    {
                        headers:
                        {
                ...
                reset()
                toast.success('Wellness Details edited successfully', {
                    position: "top-right",
                    autoClose: 5000,
            ...
            try {
                const { data } = await axios.post('/wellness/', formData,
                    {
                        headers:
                        {
                            Authorization: `Bearer ${token}`,
    ...
    const HeaderComponent = () => {
        if (mode == "add") {
            return (
                <ModalHeader className="flex flex-col gap-1">Add New Wellness</ModalHeader>
            )
        } else if (mode == "view") {
            return (
                <ModalHeader className="flex flex-col gap-1">View Wellness Details</ModalHeader>
            )
        } else {
            return (
                <ModalHeader className="flex flex-col gap-1">Edit Wellness</ModalHeader>
            )
        }
    }
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            Swal.fire({
                ...
                title: "Do you want to publish Wellness?",
                ...
                if (result.isConfirmed) {
                    const { data } = await axios.patch(`/wellness/${initialData.id}/published/`, {
                        is_published: true
                    },
                    ...
                    toast.success('Wellness Published successfully', {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: true,
                        closeOnClick: true,
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setRefresh((prev: boolean) => !prev)
                    toast.success('Wellness Unpublished', {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: true,
                        closeOnClick: true,
...
export default AddWellnessModal